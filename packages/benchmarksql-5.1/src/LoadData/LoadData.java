...
public class LoadData
{
    ...
    private static int          loadstartw;
    private static int          loadstopw;
	...
	csvNullValue    = iniGetString("csvNullValue", "NULL");
        loadstartw	= iniGetInt("LoadStartW", 1);
 	loadstopw	= iniGetInt("LoadStopW", numWarehouses);
        nextJob         = loadstartw - 1;

        if (loadstopw > numWarehouses)
 	{
 	    loadstopw = numWarehouses;
 	}
	...
	{
	    if (nextJob > loadstopw)
		job = -1;
	    else
		job = nextJob++;

            if(job > 0 && job < loadstartw)
                job = nextJob++;
	}
